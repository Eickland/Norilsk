class ProbeLab {
    ...
    setupImportModal(modalId, openBtnId, cancelBtnId, fileInputId, dropAreaId, 
                fileInfoId, fileNameId, fileSizeId, removeBtnId, submitBtnId, 
                endpoint, customUploadHandler = null) { // Добавлен новый параметр
    const modal = document.getElementById(modalId);
    const openBtn = document.getElementById(openBtnId);
    const cancelBtn = document.getElementById(cancelBtnId);
    ...
    createDynamicEditForm(probe, probeId) {
        ...
        const fieldCategories = {
            'Основная информация': ['id', 'name', 'status_id', 'priority', 'Описание'],
            ...
            'Концентрации элементов (мг/л)': ['Ca', 'Fe', 'Ni', 'Cu', 'Co','Sc','Al','Pt','Os','Ag','Ir','Pd','Rh','Au','Zn','Ru'],
            'Масса (мг)': ['mCa', 'mFe', 'mNi', 'mCu', 'mCo','mSc','mAl','mPt','mOs','mAg','mIr','mPd','mRh','mAu','mZn','mRu'],
            'Дополнительная информация': ['Кто готовил', 'Среда', 'Аналиты', 'tags','V_aliq (l)'],
        };
        
        const fieldTypes = {
            'checkbox': ['is_solid', 'is_solution'],
            'number': ['Ca', 'Fe', 'Ni', 'Cu', 'Co','Sc','Al','Pt','Os','Ag','Ir','Pd','Rh','Au','Zn','Ru',
                 'mCa', 'mFe', 'mNi', 'mCu', 'mCo','mSc','mAl','mPt','mOs','mAg','mIr','mPd','mRh','mAu','mZn','mRu',
                      'sample_mass', 'priority', 'Разбавление', 'Масса навески (g)','pH','Eh','Плотность',
                      'Масса твердого (g)','Масса Ca(OH)2 (g)','Масса CaCO3 (g)','Объем р-ра H2SO4 (ml)','Масса железных окатышей (g)','V (ml)'],
            'textarea': ['Описание'],
            'select': ['status_id'],
        };
        ...
        const fieldLabels = {
            ...
            'описание': 'Описание',
            'tags': 'Теги',
            'V_aliq (l)': 'Объем аликвоты у твердой пробы, л'
        };
    ...
    setupBatchActions() {
        const selectAll = document.getElementById('selectAllProbes');
        ...
        // Делегирование события клика на чекбоксы в таблице
        document.getElementById('probeTableBody').addEventListener('change', (e) => {
            if (e.target.classList.contains('probe-checkbox')) {
                this.updateDeleteButtonState();
            }
        });
        ...
        // Обработка удаления
        if (deleteBtn) {
            deleteBtn.addEventListener('click', () => this.deleteSelectedProbes());
        }
    }

    updateDeleteButtonState() {
        const selected = document.querySelectorAll('.probe-checkbox:checked');
        const deleteBtn = document.getElementById('deleteSelectedBtn');
    ...
    async deleteSelectedProbes() {
        const selected = Array.from(document.querySelectorAll('.probe-checkbox:checked'))
            .map(cb => parseInt(cb.dataset.probeId));

        if (!confirm(`Вы уверены, что хотите удалить ${selected.length} проб?`)) return;

        try {
            const response = await fetch('/api/probes/batch_delete', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (result.success) {
                ...
                this.renderTable();
                this.renderStats();
                document.getElementById('selectAllProbes').checked = false;
                this.updateDeleteButtonState();
            }
        } catch (error) {
            console.error('Ошибка массового удаления:', error);
            alert('Не удалось удалить пробы');
        }
    }
